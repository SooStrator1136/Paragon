 ...
 */
object SaveMapCommand : Command("SaveMap", "savemap <holding/frames/smart>") {
        ...
        when (args[0]) {
            "holding" -> {
                currentJob = backgroundThread {
                    arrayOf(
                        minecraft.player.heldItemMainhand,
            ...
            "frames" -> {
                currentJob = backgroundThread {
                    ...
                    Paragon.INSTANCE.commandManager.sendClientMessage(
                        "Finished saving all loaded maps!",
                        fromConsole
                    )
                }
            }
            ...
            "smart" -> {
                currentJob = backgroundThread {
                    frameList.addAll(
                        minecraft.world.loadedEntityList.filterIsInstance<EntityItemFrame>().filter {
                    ...
                    Paragon.INSTANCE.commandManager.sendClientMessage(
                        "Finished merging and saving all loaded maps!",
                        fromConsole
                    )
                }
            }
        ...
        while (i < frameList.size) {
            ...
            repeat(upperMerged.size - 1) {
                mergedImage = mergeImages(
                    mergedImage,
                    upperMerged[it + 1],
                    when (currentFrame.facingDirection) {
            ...
            saveLoc = if (index == 1) {
                ...
                File(
                    saveLoc.path.substring(
                        0,
                        saveLoc.path.length - (6 + (index - 1).toString().length)
                    ) + "(${index}).jpg"
                )
            }
        }
