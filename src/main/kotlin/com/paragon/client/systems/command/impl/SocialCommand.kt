        ...
        if (args.size == 1 && args[0].equals("list", ignoreCase = true)) {
            ...
            if (Paragon.INSTANCE.socialManager.players.isEmpty()) {
                Paragon.INSTANCE.commandManager.sendClientMessage(
                    TextFormatting.RED.toString() + "You haven't added anyone to your social list!",
                    fromConsole
                )
                return
            }
            for (player in Paragon.INSTANCE.socialManager.players) {
                Paragon.INSTANCE.commandManager.sendClientMessage(
                    player.name + " - " + player.relationship.textFormatting + player.relationship,
                    fromConsole
                )
            }
            ...
            runCatching {
                ...
                Paragon.INSTANCE.commandManager.sendClientMessage(
                    TextFormatting.GREEN.toString() + "Added player " + name + " to your socials list!",
                    fromConsole
                )
                ...
                Paragon.INSTANCE.commandManager.sendClientMessage(
                    TextFormatting.RED.toString() + "Invalid argument! Should be 'friend', 'neutral', or 'enemy'",
                    fromConsole
                )
            }
            ...
            Paragon.INSTANCE.commandManager.sendClientMessage(
                TextFormatting.GREEN.toString() + "Removed player " + name + " from your socials list!",
                fromConsole
            )
