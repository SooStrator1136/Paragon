    ...
    override fun onRender3D() {
        ...
        for (player in minecraft.world.playerEntities) {
            ...
            val renderValues = doubleArrayOf(
                (minecraft.renderManager as IRenderManager).hookGetRenderPosX(), (minecraft.renderManager as IRenderManager).hookGetRenderPosY(), (minecraft.renderManager as IRenderManager).hookGetRenderPosZ()
            )
            ...
            GL11.glTranslated(
                renderVec.x - renderValues[0], renderVec.y + player.height + 0.1 + (if (player.isSneaking) 0.05 else 0.08) - renderValues[1], renderVec.z - renderValues[2]
            )
            GL11.glRotated(-minecraft.renderManager.playerViewY.toDouble(), 0.0, 1.0, 0.0)
            GL11.glRotated(
                minecraft.renderManager.playerViewX.toDouble(), (if (minecraft.gameSettings.thirdPersonView == 2) -1 else 1).toDouble(), 0.0, 0.0
            )
            ...
            if (pops.value) {
                stringBuilder.append(" ").append(TextFormatting.GOLD).append("-").append(if (player is EntityFakePlayer) 0 else Paragon.INSTANCE.popManager.getPops(player))
            }
            ...
            if (outline.value) {
                drawBorder(
                    0f, 0f, width, getHeight() + if (ClientFont.isEnabled) 0 else 2, outlineWidth.value, outlineColour.value.rgb
                )
            }
                ...
                for (stack in stacks) {
                    ...
                    if (stack.item !== Items.AIR) {
                        ...
                        for (info in itemInfo) {
                            var colour = -1
                            if (info == itemInfo[0]) {
                                val green = (stack.maxDamage.toFloat() - stack.itemDamage.toFloat()) / stack.maxDamage.toFloat()
                                val red = 1 - green
