...
class ColourElement(setting: Setting<Color>, x: Float, y: Float, width: Float, height: Float) : SettingElement<Color>(setting, x, y, width, height) {
    ...
    init {
        ...
        settings.addAll(
            listOf(
                SliderElement(hueSetting, x + 2, y, width - 4, height), SliderElement(saturationSetting, x + 2, y, width - 4, height), SliderElement(brightnessSetting, x + 2, y, width - 4, height), SliderElement(alphaSetting, x + 2, y, width - 4, height), BooleanElement(rainbowSetting, x + 2, y, width - 4, height), SliderElement(rainbowSpeedSetting, x + 2, y, width - 4, height), BooleanElement(syncSetting, x + 2, y, width - 4, height)
            )
        )
    }
    ...
    override fun draw(mouseX: Float, mouseY: Float, mouseDelta: Int) {
        ...
        setting.setValue(
            Color(
                Color.HSBtoRGB(
                    hueSetting.value.toFloat() / 360f, saturationSetting.value.toFloat() / 100f, brightnessSetting.value.toFloat() / 100f
                )
        ...
        RenderUtil.drawRect(
            x, y, width, height, setting.value.integrateAlpha(if (hovered) 205f else setting.value.alpha.toFloat()).rgb
        )
