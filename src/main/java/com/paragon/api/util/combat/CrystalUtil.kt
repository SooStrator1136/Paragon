...
object CrystalUtil {
    ...
    fun calculateDamage(vec: Vec3d, entity: EntityLivingBase): Float {
        ...
        return getBlastReduction(entity, damage * if (diff == 0f) 0f else if (diff == 2f) 1f else if (diff == 1f) 0.5f else 1.5f, Explosion(mc.world, null, vec.x, vec.y, vec.z, 6f, false, true))
    }
    ...
    private fun getBlastReduction(entity: EntityLivingBase, damage: Float, explosion: Explosion): Float {
        ...
        try {
            val enchantmentModifier = MathHelper.clamp(
                EnchantmentHelper.getEnchantmentModifierDamage(entity.armorInventoryList, source),
                0,
                20
            )
