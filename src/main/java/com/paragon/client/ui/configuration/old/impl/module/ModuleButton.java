...
public class ModuleButton {
    ...
    public void renderModuleButton(int mouseX, int mouseY) {
        ...
        if (isMouseOver(mouseX, mouseY)) {
            OldPanelGUI.tooltip = module.getDescription();
        }
    }
    ...
    public boolean isMouseOver(int mouseX, int mouseY) {
        return OldPanelGUI.isInside(getPanel().getX(), getOffset(), getPanel().getX() + getPanel().getWidth(), getOffset() + getHeight(), mouseX, mouseY);
    }
    ...
    public void refreshSettingOffsets() {
        ...
        for (SettingComponent<?> settingComponent : settingComponents) {
            if (settingComponent.getSetting().isVisible()) {
                ...
                settingOffset += settingComponent.getHeight() * animation.getAnimationFactor();
                ...
                if (settingComponent.animation.getAnimationFactor() > 0) {
                    ...
                    for (SettingComponent<?> settingComponent1 : settingComponent.getSettingComponents()) {
                        if (settingComponent1.getSetting().isVisible()) {
                            ...
                            subsettingOffset += settingComponent1.getHeight() * (float) settingComponent.animation.getAnimationFactor();
