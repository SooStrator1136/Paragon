...
class EnumElement(setting: Setting<Enum<*>>, module: ModuleElement, x: Float, y: Float, width: Float, height: Float) : SettingElement<Enum<*>>(setting, module, x, y, width, height) {
    ...
    override fun draw(mouseX: Float, mouseY: Float, mouseDelta: Int) {
        ...
        RenderUtil.scaleTo(x + width - FontUtil.getStringWidth(StringUtil.getFormattedText(setting.value)), y + 3, 0f, 0.7, 0.7, 0.7) {
            ...
            FontUtil.drawStringWithShadow(StringUtil.getFormattedText(setting.value), side, y + 5 - (3 * hover.getAnimationFactor()).toFloat(), Color.GRAY.brighter().fade(Color.GRAY.brighter().brighter(), hover.getAnimationFactor()).rgb)
        }
