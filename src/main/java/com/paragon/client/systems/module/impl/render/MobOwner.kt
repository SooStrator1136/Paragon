    ...
    override fun onRender3D() {
        minecraft.world.loadedEntityList.forEach(Consumer { entity: Entity ->
            if (entity is IEntityOwnable) {
                ...
                val owner = if ((entity as IEntityOwnable).owner == null) "No Owner" else Objects.requireNonNull((entity as IEntityOwnable).owner)?.name
                val vec = EntityUtil.getInterpolatedPosition(entity)

                if (entity is AbstractHorse) {
                    drawNametagText("Name: " + name + ", Speed: " + (entity as AbstractHorse).aiMoveSpeed + ", Owner: " + owner, Vec3d(vec.x, vec.y + 1.25, vec.z), -1)
                } else {
